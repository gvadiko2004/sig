...
(function () {
  ...
  function animateCounter(el, duration = 1200) {
    ...
    const num = parseFloat(original.replace(/[^0-9.]/g, "")); // 123.45 из "123.45K"
    if (isNaN(num)) return;
    const suffix = original.replace(/[0-9.]/g, ""); // "K" из "123.45K"
  ...
  // ===== Utils: progress bar в портфолио =====
  ...
  // ===== Прелоадер: дождаться .load и запустить callback =====
  ...
  // ===== Точка старта секции (в 2 раза раньше) =====
  // Было: mob 0.95, desk 0.30 → Стало: mob 0.475, desk 0.15
  function triggerPoint(section) {
    const rect = section.getBoundingClientRect();
    if (window.innerWidth <= 440) return rect.top <= window.innerHeight * 0.475;
    return rect.top <= window.innerHeight * 0.15;
  }

  /****************************************
   * HEADER (после preloader.load)
   ****************************************/
  function initHeader() {
    ...
    waitPreloader(() => setShow(header, 600));
  }
  document.addEventListener("DOMContentLoaded", initHeader);

  /****************************************
   * HERO (после preloader.load)
   ****************************************/
  function initHero() {
      ...
      entries.forEach((entry) => {
        ...
        const plan = {
          ...
          about: () => {
            ...
            setShow(sec, 700);
            items.forEach((it, i) =>
              setTimeout(() => setShow(it, 500), i * 120)
            );
          },
  ...
  /****************************************
   * Регистрация секций (init + observe)
   ****************************************/
  ...
  /****************************************
   * Дополнительно: Portfolio отдельный observer (как было)
   ****************************************/
  (function initPortfolioOnce() {
